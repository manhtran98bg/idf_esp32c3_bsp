 ...
 */
typedef enum {
    BSP_BTN_PRESS = GPIO_NUM_NC,
} bsp_button_t;
/** @} */ // end of buttons

#define BSP_ENCODER_A         (GPIO_NUM_NC)
#define BSP_ENCODER_B         (GPIO_NUM_NC)
...
/** @defgroup g02_storage_littlefs SD Card and LittleFS
 *  @brief LittleFS and SD card BSP API
 *  @{
 */
 ...
 * LittleFS
 *
 * After mounting the LittleFS, it can be accessed with stdio functions, e.g.:
 * \code{.c}
 * FILE* f = fopen(BSP_LITTLEFS_MOUNT_POINT "/hello.txt", "w");
 * fprintf(f, "Hello LittleFS!\n");
 * fclose(f);
 * \endcode
 **************************************************************************************************/
#define BSP_LITTLEFS_MOUNT_POINT     "/littlefs"
#define BSP_LITTLEFS_PARTITION_LABEL "littlefs"
/**
 * @brief Mount LittleFS to the virtual file system
 *
 * @return
 *      - ESP_OK on success  
 *      - ESP_ERR_INVALID_STATE if esp_vfs_littlefs_register was already called  
 *      - ESP_ERR_NO_MEM if memory cannot be allocated  
 *      - ESP_FAIL if the partition cannot be mounted  
 *      - other error codes
 */
esp_err_t bsp_littlefs_mount(void);

/**
 * @brief Unmount LittleFS from the virtual file system
 *
 * @return
 *      - ESP_OK on success  
 *      - ESP_ERR_NOT_FOUND if the partition table does not contain a LittleFS partition with the given label  
 *      - ESP_ERR_INVALID_STATE if esp_vfs_littlefs_unregister was already called  
 *      - ESP_ERR_NO_MEM if memory cannot be allocated  
